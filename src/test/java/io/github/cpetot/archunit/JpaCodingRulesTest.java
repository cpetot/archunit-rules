...
class JpaCodingRulesTest {
	...
	class JpaEntitiesHavePublicEmptyConstructor {
		...
		void should_raise_no_error_with_explicit_public_empty_constructor() {
			...
			JpaCodingRules.JPA_ENTITIES_HAVE_PUBLIC_EMPTY_CONSTRUCTOR.check(classes);
		}
		...
		void should_raise_one_error_with_public_arguments_constructor() {
			...
			Assertions.assertThatThrownBy(() -> JpaCodingRules.JPA_ENTITIES_HAVE_PUBLIC_EMPTY_CONSTRUCTOR.check(classes))
